...
mDataPoints(), mDataClusters(), mModels(), mDistancesToBeUpdated(), mKDTree(){
...
std::list<sPtLnkPointer> JLinkage::FindKNearest(sPtLnkPointer __V, float const __Max_R, int k, int nMaxSize){
		...
		while(msSizePRAF < k && msSizePRAF < nMaxSize ){
			std::pair<KDTree::KDTree<3,sPtLnkPointer>::iterator,float> nif = mKDTree.find_nearest_if(__V,__Max_R,sPredicateAlreadyFoundJL());
