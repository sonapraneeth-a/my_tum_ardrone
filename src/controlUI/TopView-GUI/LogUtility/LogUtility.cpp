...
void setLog()
{
	...
	printLogMessage(1, "[ LOG] LOG IS ACTIVATED.\n");
}
...
void setLog(int level)
{
	LOG_ACTIVATE = true; logMessage.str(string());
	LOG_LEVEL = level;
	LOG_MSG << "[ LOG] LOG IS ACTIVATED WITH LEVEL " << LOG_LEVEL << ".\n";
...
void setLog(string filename)
{
	...
	LOG_MSG << "[ LOG] LOG TO FILE \"" << logFile << "\" ACTIVATED.\n";
	cout << "[ LOG] LOG TO FILE \"" << logFile << "\" ACTIVATED.\n";
	printLogMessage(1, logMessage);
}
...
{
	LOG_ACTIVATE = false;
	printLogMessage(1, "[ LOG] LOG DEACTIVATED.\n");
